...
void replacePi(char* inp, int i) {
	...
	// base case
	if(inp[i] == '\0') {
		// you've replace all the pis with 3.14 in given inp therefore stop
		return;
	}

	// recursive case

	// replace pis with 3.14 in  inp[i...n-1]
	...
	if(inp[i] == 'p' and inp[i+1] == 'i') {
		...
		// ask your friend to replace pis with 3.14 in inp[i+4....]

		replacePi(inp, i+4);

	} else {

		// ask your friend to replace pis with 3.14 in inp[i+1....]
...
int main() {
	...
	char inp[20] = "pip"; // based on the constraints
