...
bool canPlaceDigit(int grid[][9], int n, int i, int j, int d) {
	...
	for(int k=0; k<n; k++) {
		if(grid[i][k] == d || grid[k][j] == d) {
			// 'd' is already present along the ith  row or the jth column
			return false;
		}
	}
	...
	for(int x=s_x; x<s_x+rn; x++) {
		for(int y=s_y; y<s_y+rn; y++) {
			if(grid[x][y] == d) {
				// 'd' is present in the subgrid of (i, j)th cell
				return false;
			}
		}
	}

	// you can place 'd' in the (i, j)th cell
	return true;
