...
TreeNode* buildBBST(int* A, int s, int e) {
	...
	if(s > e) {
		return NULL; // you've to build an empty tree
	}

	int m = s+(e-s)/2;
	TreeNode* root = new TreeNode(A[m]);

	// 1. build the leftSubtree which should be a balanced BST using A[s...m-1] {sorted}

	root->left = buildBBST(A, s, m-1);

	// 2. build the rightSubtree which should be a balanced BST using sorted A[m+1...e]
