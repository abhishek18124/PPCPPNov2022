...
int minSteps(int n) {
	...
	// option 1 : reduce n to n-1 -> ask your friend to find min. steps required to reduce n-1 to 1
	...
	int y = INT_MAX;
	if(n%2 == 0) {
		// ask your friend to find min. steps required to reduce n/2 to 1
		y = minSteps(n/2);
	}
	...
	int z = INT_MAX;
	if(n%3 == 0) {
		// ask your friend to find min. steps required to reduce n/3 to 1
		z = minSteps(n/3);
	}
