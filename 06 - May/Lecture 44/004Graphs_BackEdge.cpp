		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				if (dfsHelper(neighbour, visited, stackState)) {
					// you've found a back-edge in the sub-component
					return true;
				}
				...
				// check source->neigbour is it a back-edge ?

				// check if there is directed path from neighbour to source

				if (stackState.find(neighbour) != stackState.end()) {
					// there is a directed path from neighbour to source therefore source to neighbour is a back edge
					return true;
				}
			}
		}
