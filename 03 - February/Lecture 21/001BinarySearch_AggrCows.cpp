...
bool canPlaceCows(int c, int n, int* pos, int m) {
	...
	// place the 1st cow in the 1st stall i.e. pos[0]
	...
	for(int i=1; i<n; i++) {
		...
		if(pos[i]-prevCowPos >= m) {
			...
			if(numOfCowsPlaced == c) {
				// you've mananged to place c cows in n stalls at a min.dist. of >= m
				return true;
			}
		}
	}

	// you cannot place c cows in n stalls at a min. dist. of >= m
	return false;
